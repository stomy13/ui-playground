...
const travelSchema = z
  .object({
    ...
    ageGroups: z.array(z.enum(['child', 'adult', 'senior'])).min(1, '年齢層を選択してください'),
  ...
  .superRefine((data, ctx) => {
    if (data.travelType === 'international' && data.ageGroups.includes('adult')) {
...
async function submitTravelForm(
  ...
  try {
    const existingData = JSON.parse(localStorage.getItem('travelBookings') || '[]')
    const newData = {
      id: Date.now(),
      ...submission.value,
...
export function TravelFormZod() {
  ...
  const ageGroups = Array.isArray(fields.ageGroups.value) ? fields.ageGroups.value : []

  return (
    <Box component="form" {...getFormProps(form)} action={formAction} sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
      ...
      <FormControl component="fieldset" error={!!fields.ageGroups.errors}>
        ...
        <FormGroup>
          <FormControlLabel
            control={<Checkbox {...getInputProps(fields.ageGroups, { type: 'checkbox', value: 'child' })} />}
            label="子供"
          />
          <FormControlLabel
            control={<Checkbox {...getInputProps(fields.ageGroups, { type: 'checkbox', value: 'adult' })} />}
            label="大人"
          />
          <FormControlLabel
            control={<Checkbox {...getInputProps(fields.ageGroups, { type: 'checkbox', value: 'senior' })} />}
            label="高齢者"
          />
        </FormGroup>
      ...
      {travelType === 'international' && ageGroups.includes('adult') && (
        <Box sx={{ p: 2, bgcolor: 'grey.50', borderRadius: 1 }}>
          <Typography variant="h6" gutterBottom={true}>海外旅行情報</Typography>
          <FormGroup>
            <FormControlLabel
              control={<Checkbox {...getInputProps(fields.passportRequired, { type: 'checkbox' })} />}
              label="パスポート情報入力済み"
            />
            <FormControlLabel
              control={<Checkbox {...getInputProps(fields.visaRequired, { type: 'checkbox' })} />}
              label="ビザが必要"
            />
          </FormGroup>
          {fields.passportRequired.errors && (
            <FormHelperText error={true}>{fields.passportRequired.errors}</FormHelperText>
          )}
        </Box>
      )}
      ...
      {ageGroups.includes('child') && (
        <Box sx={{ p: 2, bgcolor: 'grey.50', borderRadius: 1 }}>
          <Typography variant="h6" gutterBottom={true}>お子様情報</Typography>
          <FormControl component="fieldset">
            <FormLabel component="legend">年齢範囲（複数選択可）</FormLabel>
            <FormGroup>
              <FormControlLabel
                control={<Checkbox {...getInputProps(fields.childAgeRanges, { type: 'checkbox', value: '0-2' })} />}
                label="0-2歳"
              />
              <FormControlLabel
                control={<Checkbox {...getInputProps(fields.childAgeRanges, { type: 'checkbox', value: '3-6' })} />}
                label="3-6歳"
              />
              <FormControlLabel
                control={<Checkbox {...getInputProps(fields.childAgeRanges, { type: 'checkbox', value: '7-12' })} />}
                label="7-12歳"
              />
            </FormGroup>
          </FormControl>
          <FormControl component="fieldset" sx={{ mt: 2 }}>
            ...
            <FormGroup>
              <FormControlLabel
                control={<Checkbox {...getInputProps(fields.childServices, { type: 'checkbox', value: 'babysitting' })} />}
                label="ベビーシッター"
              />
              <FormControlLabel
                control={<Checkbox {...getInputProps(fields.childServices, { type: 'checkbox', value: 'kidsmeal' })} />}
                label="キッズミール"
              />
              <FormControlLabel
                control={<Checkbox {...getInputProps(fields.childServices, { type: 'checkbox', value: 'playground' })} />}
                label="キッズプレイエリア"
              />
            </FormGroup>
          </FormControl>
      ...
      {ageGroups.includes('senior') && (
        <Box sx={{ p: 2, bgcolor: 'grey.50', borderRadius: 1 }}>
          <Typography variant="h6" gutterBottom={true}>バリアフリー対応</Typography>
          <FormControl component="fieldset">
            <FormLabel component="legend">対応要望（複数選択可）</FormLabel>
            <FormGroup>
              <FormControlLabel
                control={<Checkbox {...getInputProps(fields.accessibilityNeeds, { type: 'checkbox', value: 'wheelchair' })} />}
                label="車椅子対応"
              />
              <FormControlLabel
                control={<Checkbox {...getInputProps(fields.accessibilityNeeds, { type: 'checkbox', value: 'elevator' })} />}
                label="エレベーター完備"
              />
              <FormControlLabel
                control={<Checkbox {...getInputProps(fields.accessibilityNeeds, { type: 'checkbox', value: 'handrails' })} />}
                label="手すり設置"
              />
            </FormGroup>
          </FormControl>
      ...
      {state.message && (
        <Box sx={{ p: 2, bgcolor: state.success ? 'success.light' : 'error.light', borderRadius: 1 }}>
